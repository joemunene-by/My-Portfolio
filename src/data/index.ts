export const projects = [
  ...
  {
    name: "ghostloop",
    description: "The agent loop, embodied. A tool-using agent runtime, fail-closed safety policy pipeline, and sim-first execution harness for embodied AI / robotics. Sister project to GhostLM that takes the same GhostAgent shape (tool registry, policy gates, structured trace, paired-comparison eval) and binds it to robot motion primitives instead of CVE / MITRE / CWE lookups. v0.3.0 ships: PyBulletBackend + MuJoCoBackend + MockBackend (backend-agnostic at the Intent layer); a MuJoCo Menagerie loader for one-line access to Franka Panda / UR5e / Stretch / Allegro / Spot / Aloha; LLMPolicy adapter for any OpenAI-compatible endpoint (Ollama / OpenAI / Anthropic / Gemini / Ollama / vLLM); a bench harness with Wilson 95% CIs + McNemar exact p + Cohen's h paired comparisons; trace replay tooling; five policy gates (DenyList / RateLimit / Geofence / ForceCap / HumanInTheLoop); Episode catalogue with 4 builders + 3 presets; and a `python -m ghostloop` CLI. 93 tests green, zero runtime dependencies in core.",
    language: "Python",
    stars: 0,
    topics: ["robotics", "embodied-ai", "agent", "vla", "tool-use", "policy-gates", "safety", "mujoco", "pybullet", "ros2", "mcp", "sim-first"],
...
export const resumeData = {
  ...
  experience: [
    ...
    },
    {
      ...
      points: [
        ...
        "Built ghostloop from scratch — the embodied-AI sister project that takes the GhostAgent shape (tool registry, policy gates, structured trace, paired-comparison eval) and binds it to robot motion primitives. v0.3.0 ships three backends (MockBackend, MuJoCoBackend with a Menagerie loader resolving Franka / UR5e / Stretch / Allegro / Spot / Aloha, PyBulletBackend), an LLMPolicy adapter for any OpenAI-compatible endpoint, a bench harness with Wilson CIs + McNemar exact p + Cohen's h, trace replay tooling, five policy gates (DenyList / RateLimit / Geofence / ForceCap / HumanInTheLoop), an Episode catalogue with reusable builders, and a `python -m ghostloop` CLI. 93 tests green, zero runtime deps in core.",
  ...
  highlights: [
    ...
    "Built ghostloop — an embodied-AI agent runtime + safety pipeline + bench harness for robotics. Three backends (MuJoCo / PyBullet / Mock), MuJoCo Menagerie loader, LLMPolicy adapter, five policy gates, paired-comparison statistics, CLI. 93 tests, v0.3.0 in one push session",
